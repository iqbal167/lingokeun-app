...
class UserProfileManager:
    ...
    def extract_weaknesses_from_review(self, review_content: str, task_type: str, date: str) -> Dict[str, List[str]]:
        ...
        grammar_patterns = {
            ...
            "tenses": ["tense", "simple present", "simple past", "simple future", "parallelism"],
    ...
    def update_weaknesses(self, review_content: str, task_type: str, date: str) -> None:
        ...
        weaknesses = self.extract_weaknesses_from_review(review_content, task_type, date)
        ...
        for vocab in weaknesses["vocabulary"]:
            existing_vocab = next((v for v in profile["vocabulary_gaps"] if v["word"] == vocab), None)
            if existing_vocab:
        ...
        profile["review_history"].append({
            "date": date,
            "task_type": task_type,
            "weaknesses_found": len(weaknesses["grammar"]) + len(weaknesses["translation"]) + len(weaknesses["vocabulary"])
        })
    ...
    def _is_improving(self, history: List[Dict]) -> bool:
        ...
        recent_3 = history[-3:]
        return all(recent_3[i]["count"] >= recent_3[i+1]["count"] for i in range(len(recent_3)-1))
    ...
    def get_user_context_for_ai(self) -> str:
        ...
        if profile["focus_areas"]["urgent"]:
            context_parts.append(f"URGENT areas to focus on: {', '.join(profile['focus_areas']['urgent'])}")
        
        # Persistent issues
        if profile["patterns"]["persistent_issues"]:
            context_parts.append(f"Persistent issues (3+ mistakes): {', '.join(profile['patterns']['persistent_issues'])}")
        ...
        if profile["patterns"]["improving_areas"]:
            context_parts.append(f"Improving areas: {', '.join(profile['patterns']['improving_areas'])}")
        
        return "\n".join(context_parts) if context_parts else "User showing good progress overall."